...
1. ### Signing files in the bundle
    ...
    This command generates a hash from each file in the `foo/` bundle folder:
    ```bash
    # Go to the bundle/ directory
    cd bundle
    # Sign the content in the foo directory 
    opa sign --signing-key ../private.pem --bundle foo foo/
